...
export default function App() {
  ...
  // Optimistic UI Deletion Logic
  const deleteChat = async (e, chatId) => {
    e.stopPropagation();

    // 1. Immediately calculate the updated UI list locally
    const updatedChats = chats.filter((chat) => chat.id !== chatId);
    const wasActive = activeChat === chatId;

    // 2. Perform optimistic local state updates right away
    if (wasActive) {
      if (updatedChats.length > 0) {
        setActiveChat(updatedChats[0].id);
      } else {
        // Temp local fallback ID so the UI has an active container to show
        setActiveChat("temp-fallback-id");
        updatedChats.push({ id: "temp-fallback-id", title: "New Chat" });
      }
    }
    setChats(updatedChats);

    // 3. Fire the database network operations in the background
    try {
      const chatRef = doc(db, "users", user.uid, "chats", chatId);
      await deleteDoc(chatRef);

      // If the list became completely empty, write the official fallback to Firestore
      if (
        updatedChats.length === 1 &&
        updatedChats[0].id === "temp-fallback-id"
      ) {
        const chatsRef = collection(db, "users", user.uid, "chats");
        const docRef = await addDoc(chatsRef, {
          title: "New Chat",
        ...
        // Swap the temporary client ID out for the permanent database production ID
        setChats([{ id: docRef.id, title: "New Chat" }]);
        setActiveChat(docRef.id);
      }
    } catch (error) {
      console.error("Failed to delete backend record:", error);
      // Optional: Re-fetch or handle error rollbacks here if the request fails
    }
  };
  ...
  return (
    <div className="h-screen flex bg-[#070a0e] text-slate-100 font-sans overflow-hidden antialiased relative">
      {sidebarOpen && (
        <div
          onClick={() => setSidebarOpen(false)}
          className="fixed inset-0 bg-black/60 backdrop-blur-sm md:hidden z-40 transition-all duration-300"
        />
      )}
      ...
      >
        <div className="flex items-center justify-between mb-6 px-1 select-none">
          ...
          <button
            onClick={() => setSidebarOpen(false)}
            className="md:hidden text-slate-400 hover:text-white p-2 text-3xl font-light leading-none touch-manipulation cursor-pointer select-none"
            title="Close Sidebar"
          >
            &times;
          </button>
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="opacity-100 md:opacity-0 md:group-hover:opacity-100 ml-2 p-2 rounded-md text-slate-400 md:text-slate-100 hover:text-red-400 hover:bg-white/5 transition-all duration-150 text-xl md:text-base leading-none cursor-pointer touch-manipulation z-10"
                  title="Delete Chat"
                >
                  &times;
                </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#0b0f17] relative z-10">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#090d14]/30 backdrop-blur-md h-18.25">
          <div className="flex items-center gap-2 md:hidden w-full">
            <button
              ...
              className="p-2 -ml-2 rounded-xl hover:bg-white/5 transition text-slate-300 cursor-pointer text-xl font-bold z-20"
            >
              ☰
            </button>
            <h1 className="flex items-center gap-2 text-lg font-semibold tracking-wide text-slate-300 min-w-0 flex-1">
