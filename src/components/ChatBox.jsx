...
const getGreeting = (name) => {
  ...
  if (hour < 12)
    return { text: `Good Morning${personalGreeting}`, emoji: "🌤️" };
  if (hour < 17)
    return { text: `Good Afternoon${personalGreeting}`, emoji: "☀️" };
  if (hour < 21)
    return { text: `Good Evening${personalGreeting}`, emoji: "🌆" };
  return {
    text: name ? `Just You & Me, ${name}` : "Just You & Me",
    emoji: "🤝",
  };
};
...
export default function ChatBox({ activeChatId, onFirstMessage }) {
  ...
  // Get current Firebase user details for greeting personalization
  ...
  useEffect(() => {
    ...
    recognition.onend = () => {
      ...
      // Automatically triggers the submit process instantly when mic turns off
      setTimeout(() => {
    ...
    if (isListening) {
      ...
      setTimeout(() => {
        try {
          ...
          console.warn(
            "Recognition engine already handling an active stream:",
            err.message,
          );
        }
      }, 250);
  ...
  const sendMessage = async (textOverride) => {
    const userText = (
      textOverride !== undefined && typeof textOverride === "string"
        ? textOverride
        : input
    ).trim();
            ...
            <div className="w-14 h-14 rounded-2xl bg-linear-to-tr from-cyan-500/20 to-blue-600/5 border border-cyan-500/30 flex items-center justify-center text-cyan-400 shadow-xl shadow-cyan-950/20">
              ...
              >
                <path
                  ...
                  d="M9.813 15.904L9 21l8.954-8.955M21 12h-6m6 0a9 9 0 11-18 0 9 9 0 0118 0z"
                />
              </svg>
            </div>
      ...
      <div className="p-4 bg-linear-to-t from-[#0e1118] via-[#0e1118] to-transparent">
              ...
              {ttsEnabled ? (
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M15.536 8.464a5 5 0 010 7.072M12 6v12m0 0l-3-3m3 3l3-3M9 9H5a1 1 0 00-1 1v4a1 1 0 001 1h4l3 3V6L9 9z"
                  />
                </svg>
              ) : (
                <svg
        ...
        <div className="relative flex items-end bg-[#131722] border border-white/6 focus-within:border-cyan-500/40 focus-within:ring-4 focus-within:ring-cyan-500/3 rounded-2xl p-2 transition-all duration-300 shadow-2xl">
          <textarea
            ...
            disabled={isLoading}
            // Mobile Input Optimization Attributes:
              ...
              {isListening ? (
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M9 9v6m3-8v10m3-7v4M3 12h1m17 0h-1M5.5 7.5l.7.7m12.3-.7-.7.7M5.5 16.5l.7-.7m12.3.7-.7-.7"
                  />
                </svg>
              ) : (
                <svg
        ...
        {(isListening || speechError) && (
          <p
            className={`text-xs text-center mt-2 animate-fade-in ${speechError ? "text-amber-400" : "text-cyan-400/70"}`}
          >
